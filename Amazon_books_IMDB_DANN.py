...
Amz_books_x_text, Amz_books_y = load_data_and_labels(BASE_DIR + '/Data/Amazon_reviews/books/pos_books.txt', BASE_DIR + '/Data/Amazon_reviews/books/neg_books.txt')
Amz_books_max_document_length = max([len(x.split(" ")) for x in Amz_books_x_text])

# Load target domain dataset IMDB
IMDB_x_text, IMDB_y = load_data_and_labels(BASE_DIR + '/Data/IMDB/pos_movies.txt', BASE_DIR + '/Data/IMDB/neg_movies.txt')
IMDB_max_document_length = max([len(x.split(" ")) for x in IMDB_x_text])

seq_length = (Amz_books_max_document_length + IMDB_max_document_length) // 2
...
num_test = 500
combined_test_txt = np.vstack([Amz_books_x_test[:num_test], IMDB_x_test[:num_test]])
...
class MNISTModel(object):
    ...
    def _build_model(self):
        self.X_length=Amz_books_x_train.shape[1]
...
def train_and_evaluate(training_mode, graph, model, verbose=False):
    ...
    with tf.Session(graph=graph) as sess:
        summary_writer = tf.summary.FileWriter('Amazon_books_To_IMDB_log_DANN_networks', sess.graph)
        ...
        gen_target_batch = batch_generator(
            [IMDB_x_train, Amz_books_y_train], batch_size // 2) #here they use hte labels of hte source training along wiht samples fro mthe target . instead of the target labels
        gen_source_only_batch = batch_generator(
            [Amz_books_x_train, Amz_books_y_train], batch_size)
        gen_target_only_batch = batch_generator(
            [IMDB_x_train, Amz_books_y_train], batch_size)
        ...
        source_acc = sess.run(label_acc,
                            feed_dict={model.X: Amz_books_x_test, model.y: Amz_books_y_test,
                                       model.train: False})

        target_acc = sess.run(label_acc,
                            feed_dict={model.X: IMDB_x_test, model.y: Amz_books_y_test,
                                       model.train: False})
...
print('Source (Amazon movies DS) accuracy:', source_acc)
print('Target (IMDB) accuracy:', target_acc)
