...
# Load target domain dataset amazon movies
...
seq_length = (Amz_books_AVG_document_length + Amz_movies_AVG_document_length) // 2
...
num_test = 500
combined_test_txt = np.vstack([Amz_books_x_test[:num_test], Amz_movies_x_test[:num_test]])
combined_test_labels = np.vstack([Amz_books_y_test[:num_test], Amz_books_y_test[:num_test]]) # they use hte labels of hte source domian dataset with the target domain dataset
...
def train_and_evaluate(training_mode, graph, model, verbose=False):
    ...
    with tf.Session(graph=graph) as sess:
        ...
        gen_target_batch = batch_generator(
            [Amz_movies_x_train, Amz_movies_y_train], batch_size // 2) #here they use hte labels of hte source training along wiht samples fro mthe target . instead of the target labels
