...
MR_x_text, MR_y = load_data_and_labels(BASE_DIR + '/Data/MR/rt-polaritydata/rt-polarity.pos', BASE_DIR + '/Data/MR/rt-polaritydata/rt-polarity.neg')

# Build vocabulary
max_document_length = max([len(x.split(" ")) for x in MR_x_text])
print(max_document_length)
vocab_processor = learn.preprocessing.VocabularyProcessor(max_document_length)
MR_x = np.array(list(vocab_processor.fit_transform(MR_x_text)))

# Randomly shuffle data
np.random.seed(10)
shuffle_indices = np.random.permutation(np.arange(len(MR_y)))
MR_x_shuffled = MR_x[shuffle_indices]
MR_y_shuffled = MR_y[shuffle_indices]

# Load target domain dataset IMDB
IMDB_x_text, IMDB_y = load_data_and_labels(BASE_DIR + '/Data/MR/rt-polaritydata/rt-polarity.pos', BASE_DIR + '/Data/MR/rt-polaritydata/rt-polarity.neg')

# Build vocabulary
max_document_length = max([len(x.split(" ")) for x in IMDB_x_text])
vocab_processor = learn.preprocessing.VocabularyProcessor(max_document_length)
...
dev_sample_index = -1 * int(0.1 * float(len(MR_y)))

MR_x_train, MR_x_test = MR_x_shuffled[:dev_sample_index], MR_x_shuffled[dev_sample_index:]
MR_y_train, MR_y_test = MR_y_shuffled[:dev_sample_index], MR_y_shuffled[dev_sample_index:]
...
num_test = 500
combined_test_txt = np.vstack([MR_x_test[:num_test], IMDB_x_test[:num_test]])
combined_test_labels = np.vstack([MR_y_test[:num_test], MR_y_test[:num_test]]) # they use hte labels of hte source domian dataset with the target domain dataset
...
class MNISTModel(object):
    ...
    def _build_model(self):
        self.X_length=MR_x_train.shape[1]
        self.y_length =MR_y_train.shape[1]
...
def train_and_evaluate(training_mode, graph, model, verbose=False):
    ...
    with tf.Session(graph=graph) as sess:
        summary_writer = tf.summary.FileWriter('log_DANN_networks', sess.graph)
        ...
        gen_source_batch = batch_generator(
            [MR_x_train, MR_y_train], batch_size // 2)
        gen_target_batch = batch_generator(
            [IMDB_x_train, MR_y_train], batch_size // 2) #here they use hte labels of hte source training along wiht samples fro mthe target . instead of the target labels
        gen_source_only_batch = batch_generator(
            [MR_x_train, MR_y_train], batch_size)
        gen_target_only_batch = batch_generator(
            [IMDB_x_train, MR_y_train], batch_size)
        ...
        source_acc = sess.run(label_acc,
                            feed_dict={model.X: MR_x_test, model.y: MR_y_test,
                                       model.train: False})

        target_acc = sess.run(label_acc,
                            feed_dict={model.X: IMDB_x_test, model.y: MR_y_test,
                                       model.train: False})
...
print('Source (MR DS) accuracy:', source_acc)
