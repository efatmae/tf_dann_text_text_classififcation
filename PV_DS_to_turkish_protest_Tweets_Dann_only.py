...
test_set = open(os.path.join(BASE_DIR, "results/DA_test_set_vocab.txt"), "w")
#PV_vocab = open(os.path.join(BASE_DIR, "results/PV_features_vocab.txt"), "w")
#tweets_vocab = open(os.path.join(BASE_DIR, "results/Turkish_tweets_features_vocab.txt"), "w")
#CF_tweets_vocab = open(os.path.join(BASE_DIR, "results/CF_tweets_features_vocab.txt"), "w")
# Process source domain dataset PV dataset
PV_DS_x_text, PV_DS_y = tfp.load_data_and_labels(
...
dev_sample_index = -1 * int(0.1 * float(len(PV_DS_y_shuffled)))
...
vocab_size = vocab_processor_PV.vocabulary_ #vocab_processor_tweets.vocabulary_   #
...
#return vocabulary with index
def write_features(vocanProcessor, file):
    vocab_dic = vocanProcessor.vocabulary_._mapping
    sorted_vocab = sorted(vocab_dic.items(), key = lambda x : x[1])
    vocabulary = list(list(zip(*sorted_vocab)))
    file.write("\n".join(map(lambda x: str(x), vocabulary)))



#write_features(vocab_processor_PV, PV_vocab)
#write_features(vocab_processor_tweets, tweets_vocab)
#write_features(vocab_processor_tweets_test, CF_tweets_vocab)

batch_size = 128 #len(CF_tweets_DS_x_shuffled) + len(tweets_x_text)
batch_source_ratio = 50 * batch_size // 100 #len(CF_tweets_DS_x_train)
batch_target_ratio = 50 * batch_size // 100 #len(tweets_x_text)
...
def train_and_evaluate(training_mode, graph, config, model, verbose=False):
    ...
    with tf.Session(graph=graph, config=config) as sess:
        ...
        for i in range(num_steps):
            ...
            if training_mode == 'dann':
                ...
                X = np.vstack([X0, X1])
                y = np.vstack([y0, y1])
                _, batch_loss, dloss, ploss, d_acc, p_acc, p_acc2, p_auc = sess.run(
                    [dann_train_op, total_loss, domain_loss, pred_loss, domain_acc, label_acc, label_acc2, label_auc],
        ...
        src_label_acc = sess.run(label_acc2,
                            feed_dict={model.X: PV_DS_x_test, model.y: PV_DS_y_test,
                                       model.train: False})
        ...
        trgt_label_acc = sess.run(label_acc2,
                            feed_dict={model.X: tweets_x_test, model.y: tweets_y_test,
                                       model.train: False})
    ...
    return source_acc, src_label_acc, source_auc, src_pred2, src_actual2, target_acc, trgt_label_acc, target_auc, pred_prob2, pred2, actual2, loss2, test_domain_acc, test_emb


# for x in range(0, 100):
print('\nDANN only training')
config = tf.ConfigProto(allow_soft_placement=True)
source_acc, src_label_acc, source_auc, src_pred3, src_actual3, target_acc, trgt_label_acc, target_auc, pred_prob3, pred3, actual3, loss3, test_domain_acc, test_emb = train_and_evaluate(
    'dann', graph, config, model)

print('Source (PV dataset) accuracy:', source_acc)
print('Source (PV dataset) tf metrics accuracy:', src_label_acc)
print('Source (PV dataset) auc:', source_auc)
print('Source (PV dataset) sk_auc:',  metrics.roc_auc_score(src_actual3, src_pred3))
print('Target (Turkish protest Tweets) accuracy:', target_acc)
print('Target (Turkish protest Tweets) tf metrics accuracy:', trgt_label_acc)
...
print('Target (Turkish protest Tweets) loss:', loss3)
