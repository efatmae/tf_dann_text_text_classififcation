...
PV_DS_x_text, PV_DS_y = tfp.load_5000_data_and_labels(os.path.join(BASE_DIR, 'Data/PV_DS/PV_Dann_Data/pos_vio_news.txt'), os.path.join(BASE_DIR, 'Data/PV_DS/PV_Dann_Data/neg_vio_news.txt'))
    ...
    with tf.Session(graph=graph) as sess:
        ...
        for i in range(num_steps):
            ...
            if training_mode == 'dann':
                ...
                X = np.vstack([X0, X1])
                y = np.vstack([y0, y1])
                _, batch_loss, dloss, ploss, d_acc, p_acc, p_auc = sess.run([dann_train_op, total_loss, domain_loss, pred_loss, domain_acc, label_acc, label_auc],
                ...
                print('loss: %f  d_acc: %f  p_acc: %f  p_auc: %f  p: %f  l: %f  lr: %f' % (batch_loss, d_acc, p_acc, p_auc[0], p, l, lr))
