...
# Process source domain dataset amazon books
#load source 1 amazon books
Amz_books_x_text, Amz_books_y = tfp.load_data_and_labels(os.path.join(BASE_DIR, 'Data/Amazon_reviews/books/pos_books.txt'), os.path.join(BASE_DIR, 'Data/Amazon_reviews/books/neg_books.txt'))
Amz_books_x_text = Amz_books_x_text[:-1]
Amz_books_y = Amz_books_y[:-1]
print(len(Amz_books_x_text))
Amz_books_AVG_document_length = sum([len(x.split(" ")) for x in Amz_books_x_text]) // len(Amz_books_x_text)

# Load amazon source 2 amazon movies
...
seq_length = (Amz_books_AVG_document_length + Amz_movies_AVG_document_length + IMDB_AVG_document_length) // 3

print(seq_length)
# Build vocabulary
vocab_processor = learn.preprocessing.VocabularyProcessor(seq_length)
Amz_books_x = np.array(list(vocab_processor.fit_transform(Amz_books_x_text)))

# Randomly shuffle data
np.random.seed(10)
shuffle_indices = np.random.permutation(np.arange(len(Amz_books_y)))
Amz_books_x_shuffled = Amz_books_x[shuffle_indices]
Amz_books_y_shuffled = Amz_books_y[shuffle_indices]
...
dev_sample_index = -1 * int(0.1 * float(len(Amz_books_y)))
Amz_books_x_train, Amz_books_x_test = Amz_books_x_shuffled[:dev_sample_index], Amz_books_x_shuffled[dev_sample_index:]
Amz_books_y_train, Amz_books_y_test = Amz_books_y_shuffled[:dev_sample_index], Amz_books_y_shuffled[dev_sample_index:]
...
dev_sample_index = -1 * int(0.1 * float(len(IMDB_y)))
...
num_test = 500
combined_test_txt = np.vstack([Amz_books_x_test[:num_test], Amz_movies_x_test[:num_test], IMDB_x_test[:num_test]])
combined_test_labels = np.vstack([Amz_movies_y_test[:num_test], Amz_movies_y_test[:num_test], ])# they use hte labels of hte source domian dataset with the target domain dataset
combined_test_domain_1 = np.vstack([np.tile([1., -1.], [num_test, 1]), #domain labels 1st half 0 source and 2nd hald target 1
        np.tile([-1., 0.], [num_test, 1])])

combined_test_domain = np.vstack([combined_test_domain_1, np.tile([0., 1.], [num_test, 1])])
...
class MNISTModel(object):
    ...
    def _build_model(self):
        ...
        self.y_length =Amz_books_y_train.shape[1]
        ...
        with tf.variable_scope('domain_predictor'):
            ...
            d_W_fc1 = weight_variable([100, 3])
            d_b_fc1 = bias_variable([3])
...
def train_and_evaluate(training_mode, graph, model, verbose=False):
    ...
    with tf.Session(graph=graph) as sess:
        ...
        gen_source_1_batch = batch_generator(
            [Amz_books_x_train, Amz_books_y_train], batch_size // 2)
        gen_source_2_batch = batch_generator(
            [Amz_movies_x_train, Amz_movies_y_train], batch_size // 2)
        gen_target_batch = batch_generator(
            [IMDB_x_train, IMDB_y_train], batch_size // 2) #here they use hte labels of hte source training along wiht samples fro mthe target . instead of the target labels
        gen_source_1_only_batch = batch_generator(
            [Amz_books_x_train, Amz_books_y_train], batch_size)
        gen_source_2_only_batch = batch_generator(
        ...
        target_acc = sess.run(label_acc,
                            feed_dict={model.X: Amz_movies_x_test, model.y: Amz_books_y_test,
                                       model.train: False})
...
print('Source (Amazon books DS) accuracy:', source_acc)
...
plot_embedding(source_only_tsne, combined_test_labels.argmax(1), combined_test_domain.argmax(1), BASE_DIR + '/Graphs/Amzn_books_to_Amzn_movies_dann/', 'Source only')
plot_embedding(target_only_tsne, combined_test_labels.argmax(1), combined_test_domain.argmax(1), BASE_DIR + '/Graphs/Amzn_books_to_Amzn_movies_dann/', 'Target only')
plot_embedding(dann_tsne, combined_test_labels.argmax(1), combined_test_domain.argmax(1), BASE_DIR + '/Graphs/Amzn_books_to_Amzn_movies_dann/', 'Domain Adaptation')
