...
save_file_name = os.path.join(BASE_DIR,'Data/smsspamcollection/SMSSpamCollection')
text_data = []
...
label_auc,_ = tf.metrics.auc(y_target, prediction)
# Declare optimizer
...
pred = sess.run(prediction, feed_dict={x_data: texts_test.todense(), y_target: np.transpose([target_test])})
print(type(pred))
